...
static void _pixelpipe_final_histogram_waveform(dt_develop_t *dev, const float *const input, const dt_iop_roi_t *roi_in)
{
  ...
  uint16_t *restrict buf = dt_alloc_align(64, waveform_width * waveform_height * 4 * sizeof(uint16_t));
  memset(buf, 0, waveform_width * waveform_height * 3 * sizeof(uint16_t));
  ...
  // work color-wise so omp threads won't conflict
#ifdef _OPENMP
#pragma omp parallel for SIMD() default(none) \
  dt_omp_firstprivate(roi_in, bin_width, _height, waveform_width, input, buf) \
  schedule(static,bin_width*64) collapse(2) aligned(buf:64)
#endif
  for(int in_x = 0; in_x < roi_in->width; in_x++)
  {
    for(int k = 0; k < 3; k++)
    {
      ...
      {
        const float *const in = input + 4 * (in_x + in_y*roi_in->width) + (2-k);
        ...
        buf[(out_x + waveform_width * out_y) * 4 + k]++;
      }
    }
  }
...
#pragma omp parallel for SIMD() default(none) \
  ...
  schedule(static) collapse(2) aligned(buf,waveform:64)
#endif
  for(int k = 0; k < 3; k++)
  {
    for(int out_y = 0; out_y < waveform_height; out_y++)
    {
      const uint16_t *const in = buf + (waveform_width * out_y) * 4 + k;
