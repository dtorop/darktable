...
static void _pixelpipe_final_histogram(dt_develop_t *dev, const float *const input, const dt_iop_roi_t *roi_in)
{
  ...
  // FIXME: this will never happen now that final histogram occurs after gamma iop
  ...
  // FIXME: Even when scope type is waveform or vectorscope this work happens. Is this still necessary to calculate dev->histogram_max? Is there a shortcut if so?
  dev->histogram_max = 0;
  ...
  if(dev->scope_type == DT_DEV_SCOPE_VECTORSCOPE)
    // FIXME: use histogram_roi instead of roi_in if cropping to colorpicker works above and vectorscope calc can handle cropping?
    _pixelpipe_final_histogram_vectorscope(dev, (img_tmp) ? img_tmp: input, roi_in);
...
post_process_collect_info:
    ...
    if(dev->gui_attached && !dev->gui_leaving && pipe == dev->preview_pipe && (strcmp(module->op, "gamma") == 0))
    {
      ...
      // FIXME: is above comment true now that waveform is scaled via Cairo? when is input NULL?
      if(input)
      {
        // FIXME: the _pixelpipe_final_histogram() path convets to histogram profile, should this happen here -- or be called from there?
