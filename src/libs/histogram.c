...
static void _lib_histogram_process_histogram(dt_lib_histogram_t *d, const float *const input, int width, int height)
{
  ...
  // FIXME: this should happen in dt_lib_histogram_process() as it can apply to waveform and vectorscope as well: see https://github.com/darktable-org/darktable/issues/8008
                                     ...
                                     dt_colorspaces_color_profile_type_t in_profile_type, const gchar *in_profile_filename)
{
  ...
  // FIXME: do conversion in-place in the processing to save an extra buffer? -- at least for waveform, which already has to touch each pixel
...
void gui_init(dt_lib_module_t *self)
{
  ...
  else if(g_strcmp0(str, "logarithmic") == 0)
  { // update legacy conf
    d->scope_type = DT_LIB_HISTOGRAM_SCOPE_HISTOGRAM;
    dt_conf_set_string("plugins/darkroom/histogram/mode","histogram");
